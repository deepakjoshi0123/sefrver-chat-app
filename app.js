...
io.on("connect", (socket) => {
  socket.on("join", ({ email, room }, callback) => {
    ...
    console.log(socket.id, " to check : ", user);
  ...
  socket.on("1to1message", ({ message, user, usrfrnd }) => {
    console.log("userfrnd", usrfrnd);
    console.log("is it same ", usrfrnd, getSocketId(usrfrnd)[0].id);
  ...
  socket.on("acceptfrndreq", (email, usrfrnd) => {
    console.log("yesemitted", email, usrfrnd);
    console.log("How 's it's same ", usrfrnd, getSocketId(usrfrnd)[0].id);
    socket.to(getSocketId(usrfrnd)[0].id).emit("acceptfrndreq", {
      // now have one solution but its not good w.r.to security
      usrfrnd: email,
    });
  });
